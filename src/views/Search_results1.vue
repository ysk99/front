<template>
  ...
  <v-layout align-start justify-start row wrap>
      <v-flex xs12 lg1 md1>
        ...
        <a href="https://iying.online"><img :src="require('../assets/logo.png')" height="48" alt="爱影——影视聚合搜索"></a>

      </v-flex>
      <v-flex xs12 lg3 md6 v-if="query_leixing">
          <v-combobox
            v-model="keywd"
              :items="items"
              :loading="loading"
      ...
      <v-layout align-start justify-start row wrap>
        <v-flex xs12 lg8 md6>
          <v-flex xs12 lg8 md6>
            <v-btn outline color="indigo" small type="button" :input-value="query_leixing" @click="postQuery">在线</v-btn>&nbsp;&nbsp;
            <v-btn outline color="indigo" small type="button"  :input-value="query_leixing_down" @click="postQuery_download">下载</v-btn>
          </v-flex>
          <v-flex xs12 lg6 md8>
            ...
            <template v-for="result in results">
              ...
              >
                <v-list-tile-content>
                  ...
                  <!-- <v-list-tile-sub-title class="text--primary">{{ result.others  }}</v-list-tile-sub-title> -->
...
export default {
  ...
  created(){
    ...
    // this.download_pagedata = JSON.parse(window.localStorage.getItem("download_pagedata")),
    ...
    // localstorage.setItem('keywd', JSON.stringify(this.keywd))
    this.results = this.pagedata.data,
    // this.download_results = this.download_pagedata.data,
    this.total = this.pagedata.total,
    // this.download_total = this.download_pagedata.total,
    // this.rows = this.pagedata.total,
    // this.currentPage = this.pagedata.current_page,
    // this.perPage = this.pagedata.per_page,
